...
function replacerBlocks(
  ...
  try {
    ...
    switch (blockType) {
      ...
      case 'callout':
        replaced += `<blockquote class=${block.type}><h3>${
          block.title ?? ''
        }</h3>${marked(block.body)}</blockquote>`
        break
...
export default function processPathDescription(description) {
  ...
  const magicBlockRegex =
    /(?<TextBefore>[^[\]]*)\[block:(?<Type>[^\]]*)\](?<Content>.+?)\[\/block\](?<TextAfter>[^[\]]*)/gms

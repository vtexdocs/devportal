    ...
    return html`
      ${
        Object.keys(this.responses).length >= 1
          ? html`<div
              class="row response-panel-header"
              ...
              ${Object.keys(this.responses).map(
                (respStatus) =>
                  html` ${respStatus === '$$ref' // Swagger-Client parser creates '$$ref' object if JSON references are used to create responses - this should be ignored
                    ...
                    : html`
                        <div
                          class="resp-box ${this.selectedStatus === respStatus
                            ? 'active'
                            : ''}"
                          @click="${() => {
                        ...
                        >
                          ...
                          <div
                            style="display: flex; flex-direction: row; justify-content: flex-end; align-items: center; color: var(--fg)"
                          >
                            <div style="margin-right: 4px">
                              <span
                                class="dot ${this.getResponseStatusType(
                                  respStatus
                                )}"
                              ></span>
                            </div>
      ...
      ${Object.keys(this.responses).map(
        (status) => html`
          <div
            class="resp-content-container"
            id="resp-content-${status}"
            style="${this.selectedStatus === status
              ? 'display: block'
              : 'display: none'} "
          >
            <div class="resp-border resp-content">
              <div class="resp-title">
                ...
                <div
                  style="display: flex; flex-direction: row; justify-content: flex-end; align-items: center; column-gap: 4px"
                >
                  <div>
                    <span
                      class="dot ${this.getResponseStatusType(status)}"
                    ></span>
                  </div>
                  <div>
                    <span>${status}</span>
                  ...
                  : html`
                      <div class="tab-panel col">
                        ${Object.keys(this.mimeResponsesForEachStatus[status])
                          .length === 1
                          ? html`<code
                              style="font-weight:normal; margin-bottom:8px; width:min-content"
                            >
                              ${Object.keys(
            ...
            >
              ...
              >
                ${mimeRespDetails.examples.map(
                  (v) => html`<option
                    value="${v.exampleId}"
                    ?selected=${v.exampleId === mimeRespDetails.selectedExample}
                  >

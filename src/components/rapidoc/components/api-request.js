...
export default class ApiRequest extends LitElement {
  ...
  render() {
    return html`
      <div
        class="row-api regular-font request-panel ${'read focused'.includes(
          this.renderStyle
        ) || this.callback === 'true'
          ? 'read-mode'
          : 'view-mode'}"
      >
        ...
        <div class="row-api-right">
          ...
          ${securitySchemeTemplate.call(this)} ${serverTemplate.call(this)}
  ...
  exampleListTemplate(paramName, paramType, exampleList = []) {
    return html` ${exampleList.length > 0
      ? html`<span style="font-weight:bold; font-size:12px; margin-top: 10px;"
            >Example:
          </span>
  ...
  inputParametersTemplate(paramType) {
    ...
    for (const param of filteredParams) {
      ...
      if (
        !this.resolvedSpec.securitySchemes.some((e) => e.name === param.name)
      ) {
        tableRows.push(html`
          <div class="param-name ${param.deprecated ? 'deprecated' : ''}">
            ...
            <div class="param-type">
              ...
              ${param.deprecated
                ? html`<span style="color:#DC5A41;">deprecated</span>`
                : ''}
              ${param.required
                ? html`<span style="color:#DC5A41;">required</span>`
                : ''}
            </div>
          </div>

          ${this.allowTry === 'true'
            ? html` ${paramSchema.type === 'array'
                ? html` <tag-input
                    class="request-param"
                    style="width:100%"
                    ...
                    .value="${Array.isArray(example.exampleVal)
                      ? example.exampleVal
                      : example.exampleVal}"
                  >
                  </tag-input>`
                ...
                ? html` <div
                    class="tab-panel col"
                    style="border-width:0 0 1px 0; margin-top: 24px;"
                  >
                    <div
    ...
    return html` <div class="request-card">
      ...
      <hr
        style="border-top: 1px solid #E7E9EE;border-bottom:0;margin-block: 24px 0px;"
      />
      <div
        style="display:block; overflow-x:auto; max-width:100%;padding-inline: 16px;"
      >
...
${v.exampleFormat === 'text'
      ...
      >
        <div class="table-title top-gap row">
          ...
          ${this.request_body.required
            ...
            : ''}
          <code style="font-weight:normal; margin-left:5px">
            ${this.selectedRequestBodyType}</code
          >
          <span style="flex:1"></span>
          ${reqBodyTypeSelectorHtml}
        </div>
        ${this.request_body.description
          ? html`<div class="m-markdown-mal" style="margin-bottom:12px">
          ...
          ? html` <div
            ...
            >
              ...
              <div
                ...
                style=${this.activeSchemaTab === 'example'
                  ? 'display:flex;'
                  : 'display:none;'}
              >
                ${reqBodyExampleHtml}
  ...
  formDataTemplate(schema, mimeType, exampleValue = '') {
    ...
    if (schema.properties) {
      for (const fieldName in schema.properties) {
        ...
        formDataTableRows.push(html` <tr
            title="${fieldSchema.deprecated ? 'Deprecated' : ''}"
          >
            <td style="width:${labelColWidth}; min-width:100px;">
              <div
                class="param-name ${fieldSchema.deprecated ? 'deprecated' : ''}"
              >
                        ...
                        paramSchema.allowedValues.split('┃').map(
                          ...
                          ${html` <a
                            ...
                            @click="${(e) => {
                              const inputEl = e.target
                                .closest('div')
                                .querySelector(`[data-pname="${fieldName}"]`)
                              if (inputEl) {
    ...
    return html`
      ...
      >
        ...
        <pre
          ...
          style="white-space:pre; border: none;"
        ><code>${unsafeHTML(
          Prism.highlight(
            this.codeExample.trim().replace(/\\$/, ''),
            Prism.languages[this.selectedLanguage],
  ...
  apiResponseTabTemplate() {
    ...
    return html` <button
      ...
      <div
        class="tab-panel col"
        style="border-top: 1px solid #E7E9EE; border-bottom: 1px solid #E7E9EE; margin-top: 24px;"
      >
        ${this.codeExampleTemplate('flex')}
        <div
          style="background: #F8F7FC; padding-inline: 32px;padding-block: 16px"
        >
          ${this.responseMessage
            ? html` <div
                class="row"
                style="width:100%; height:max-content; background:#E7E9EE; border-radius:2px;padding-inline:4px;margin-bottom:4px"
              >
                <div
                  style="min-width:8px;min-height:8px;width:8px;height:8px;border-radius:50%;${this
                    .responseBlobUrl || this.responseText
                    ? 'border: 1px solid #79A479;background: #E6F2E6;'
                    : 'border: 1px solid #DC4C43;background: #F0E6E4;'}"
                ></div>
                <div
                  style="margin-left:4px; color:#4A596B; font-size:12px; font-weight:500;"
                >
                  ${this.responseMessage}
                </div>
              </div>`
          ...
          ${this.responseIsBlob
            ...
            : html` ${this.responseText
                ? html` <div
                    ...
                    style="max-height:500px; flex:1; display:flex;"
                  >
                    <button
                      class="copy-code"
                    ...
                    <pre
                      style="display:flex; white-space:pre; min-height:50px; height:auto; resize:vertical; overflow:auto"
                    >
${responseContent}</pre
                    >
                  </div>`
    ...
    return html`
      ...
      >
        ...
        <!-- ${this.parameters.length > 0 || this.request_body
          ...
          : ''} 
      <button class="m-btn primary thin-border" part="btn btn-try" @click="${this
          .onTryClick}">TRY</button>
      -->
      </div>
    `
  }
  ...
  async onTryClick(e) {
    ...
    const { fetchUrl, fetchOptions, reqHeaders } = updateCodeExample.call(
      this,
      tryBtnEl
    )

...
export class Toast extends LitElement {
  ...
  firstUpdated() {
    const onFadeOut = (event) => {
      if (
        this.renderRoot.querySelectorAll('.toast-active').length > 0 &&
    ...
    this.renderRoot
      ...
      .addEventListener('animationend', onFadeOut)
    this.renderRoot
      .getElementById('toast')
      .addEventListener('webkitAnimationEnd', onFadeOut)
  }
  ...
  render() {
    return html`
      <output
        role="status"
        class="toast${this.active ? '-active' : ''}"
        id="toast"
        style="border-color: ${this.tones[this.tone]
          .borderColor}; background: ${this.tones[this.tone].bgColor}"
      >
    ...
    return [
      css`
        ...
        .toast-active {
          ...
          z-index: 1001;
          -webkit-animation: fadeIn 0.3s ease, slideIn 0.3s ease,
            fadeOut 0.3s ease 10s forwards;

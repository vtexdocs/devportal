...
function moreLanguagesTemplate(numberOfButtons, languages) {
  ...
  return html`
    <div style="position:relative;" @mouseleave="${onLeaveMoreLanguages}">
      <div class="language-show-more" @click="${onClickMoreLanguages}">
        ${threeDots()}
      </div>
      <div
        class="more-languages-dropdown"
        style=${this.showMoreLanguages
          ? 'visibility: visible; opacity: 1;'
          : 'visibility: hidden; opacity: 0;'}
      >
        ${languages.slice(numberOfButtons).map(
          (language) => html`
            <button
            ...
            >
              ${languageIcons[language.key]
                ? languageIcons[language.key]({ width: 15, height: 15 })
                : ''}
              ${language.title}
            </button>
          `
...
export default function languagePickerTemplate() {
  ...
  const languages = HTTPSnippet.availableTargets().filter((language) =>
  ...
  return html` <section
    id="language"
    part="section-language"
    class="row-api-right-box regular-font observe-me ${'read focused'.includes(
      this.renderStyle
    )
  ...
  >
    ...
    <div class="language-picker">
      <div class="language-picker-buttons">
        ${languages.slice(0, numberOfButtons).map(
          (language) => html`
            <button
              class=${language.key === this.selectedLanguage
                ? 'square-language-button selected-language'

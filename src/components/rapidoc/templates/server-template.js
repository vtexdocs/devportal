        ...
        <div class="server-vars">
          ${Object.entries(this.selectedServer.variables).map(
            (kv) => html`
              ...
              ${kv[1].description
                ? html`<div>
                    <div style="border:none; margin-top: 4px">
...
export default function serverTemplate() {
  ...
  return html` <section
    ...
    class="server-template row-api-right-box regular-font observe-me ${'read focused'.includes(
      this.renderStyle
    )
  ...
  >
    ...
    <div style="display: flex; align-items: center;" class="server-template">
      ${this.selectedServer?.computedUrl
        ? html`
            <base-url id='copy-baseURL' url='${
              this.selectedServer?.url
            }' path='${this.path}' computedUrl='${
            this.selectedServer.computedUrl
          }' .variables='${
            this.selectedServer?.variables
          }' style="width: 100%;">

...
export function applyApiKey(
  ...
  if (securityObj.scheme?.toLowerCase() === 'basic') {
    if (username) {
      finalApiKeyValue = `Basic ${Buffer.from(
        `${username}:${password}`,
        'utf8'
      ).toString('base64')}`
    }
    ...
    finalApiKeyValue = `${
      securityObj.scheme?.toLowerCase() === 'bearer' ? 'Bearer ' : ''
    }${providedApikeyVal}`
  }
  if (finalApiKeyValue) {
...
async function fetchAccessToken(
  ...
  if (sendClientSecretIn === 'header') {
    headers.set(
      'Authorization',
      `Basic ${Buffer.from(`${clientId}:${clientSecret}`, 'utf8').toString(
        'base64'
      )}`
    )
  ...
  try {
    ...
    if (resp.ok) {
      ...
      if (respDisplayEl) {
        respDisplayEl.innerHTML = `<span style="color:var(--red)">${
...
async function onInvokeOAuthFlow(
  ...
  const state = `${Math.random().toString(36).slice(2, 9)}random${Math.random()
    .toString(36)
    .slice(2, 9)}`
  const nonce = `${Math.random().toString(36).slice(2, 9)}random${Math.random()
    .toString(36)
    .slice(2, 9)}`
...
export default function securitySchemeTemplate() {
  ...
  return html`
    <section
      id="auth"
      part="section-auth"
      class="row-api-right-box observe-me ${'read focused'.includes(
        this.renderStyle
      )

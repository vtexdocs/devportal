...
function headingRenderer(tagElementId) {
  ...
  renderer.heading = (text, level, raw, slugger) =>
    `<h${level} class="observe-me" id="${tagElementId}--${slugger.slug(
      raw
    )}">${text}</h${level}>`
  return renderer
}
...
export function expandedEndpointBodyTemplate(path, tagName = '') {
  ...
  const nonEmptyApiKeys =
    this.resolvedSpec.securitySchemes.filter(
      (v) =>
        v.finalKeyValue && path.security?.some((ps) => v.securitySchemeId in ps)
    ) || []
  ...
  return html`
    ...
    >
      ...
      ${this.renderStyle === 'focused' && tagName !== 'General ⦂'
        ? html`<h3
            class="operation-tag"
            style="font-weight:bold; margin-bottom:48px"
            part="section-operation-tag"
          >
            ${tagName}
          </h3>`
        : ''}
      ...
      ${html` ${path.xBadges && path.xBadges?.length > 0
          ? html`
            ...
            `
          : ''}
        ${this.specUrl && this.allowSpecFileDownload
          ? html`<div style="position:absolute; right:0; top:28px;">
              <div
                style="display:flex; justify-content: flex-end; margin:0px 0px 32px; gap:8px; flex-wrap: wrap;"
              >
                <button
                  class="m-btn m-btn-tertiary thin-border"
                ...
                <button
                  class="m-btn m-btn-secondary thin-border"
                  part="btn btn-outline"
                  @click="${(e) => {
        ...
        ${path.isWebhook
          ...
          : html`
              <div
                class="mono-font regular-font-size label-operation-container"
                part="section-operation-webhook-method"
              >
                <div
                  class="label-operation-method-container"
                  style="border-color: var(--${path.method}-border-color); background-color: var(--${path.method}-bg-color);"
                >
                  <span
                ...
                <div class="label-operation-path-container">
                  <content-copy-button
                    id="${path.method}${path.path}"
                    content="${joinURLandPath(
